  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
          ...
          title: const Center(
            child: Text("Корзина", style: TextStyle(fontSize: 30)),
          )),
      body: Column(
        children: <Widget>[
          Container(
          ...
          ElevatedButton(
              onPressed: () {
                ...
                while(count != basketList.length){
                  historyList.add(basketList[count]);
                  count++;
                }
                basketList.clear();
                Navigator.pop(context);
                //historyList.add(basketList[index_basket]);
              },
              child: Text('Купить все',
                  style: TextStyle(
                      color: Color.fromARGB(255, 0, 255, 200), fontSize: 17)))
        ],
      ),
    );
  }
    ...
    return Container(
        ...
        child: Column(children: <Widget>[
          ...
          Expanded(
            flex: 2,
            child: Image.network(
              '${basketList[index_basket].photoPath[1]}',
          ...
          Center(
              child: Expanded(
            child:
                Text('Название автомобиля: ${basketList[index_basket].title}'),
          )),
          Center(
              child: Expanded(
            child: Text('Модель: ${basketList[index_basket].model}'),
          )),
          Center(
            child: Expanded(
                child: Text('Цена: ${basketList[index_basket].cost}₽')),
          ),
          Container(
            child: Expanded(
                flex: 1,
                child: Row(
                  children: <Widget>[
                      ...
                      child: ElevatedButton(
                          onPressed: _isDisabled
                              ...
                              : () {
                                  ...
                                  } else if (basketList[index_basket]
                                          .quantity ==
                                      2) {
                                    _isDisabled = true;
                                    setState(() {
                    ...
                    Expanded(
                        child: Text(
                      basketList[index_basket].quantity.toString(),
